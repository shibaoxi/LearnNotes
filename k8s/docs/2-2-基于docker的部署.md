...
rpm -ivh cri-dockerd-0.2.6-3.el8.x86_64.rpm
#从国内 cri-dockerd 服务无法下载 k8s.gcr.io上面相关镜像,导致无法启动,所以需要修改cri-dockerd 使用国内镜像源
sed -ri '/ExecStart.*/s@(ExecStart.*)@\1 --pod-infra-container-image registry.aliyuncs.com/google_containers/pause:latest@g' /lib/systemd/system/cri-docker.service
sed -nr '/ExecStart.*/p' /lib/systemd/system/cri-docker.service
# 重启服务
...
### 6. 部署Haproxy 和 Keepalived（在Master节点上执行）
...
```bash
docker run --cap-add=NET_ADMIN --cap-add=NET_BROADCAST --cap-add=NET_RAW --net=host --restart=always --volume /etc/keepalived/keepalived.conf:/container/service/keepalived/assets/keepalived.conf -d osixia/keepalived --copy-service
```

### 7. 初始化高可用master（在第一个Master节点执行）
...
### 9. 部署 calico 网络插件（在第一个Master节点执行）
